...
export async function entries(){const r=await db().prepare("SELECT data FROM entries").all<{data:string}>();const map=new Map<string,Entry>(([...textbook,...extra,...genki,...grammar] as Entry[]).map(e=>[e.id,e]));for(const row of r.results){const e=JSON.parse(row.data);map.set(e.id,e);}return [...map.values()];}
