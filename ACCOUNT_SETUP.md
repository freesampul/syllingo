...
Automatic RLS is enabled and automatic table exposure is disabled. Migration `202609210001_accounts.sql` was applied successfully through the dashboard SQL Editor on September 21, 2026; a follow-up query verified RLS on all 11 public tables. Do not rerun this migration. This manual application does not record Supabase CLI migration history. The server connection is configured in ignored `.env.local`. Seeding completed successfully with 2,081 entries, verified by an exact database count. Signed-out Data API access is denied. The local app reports accounts configured. Site URL is `http://localhost:3000`, with the exact `http://localhost:3000/auth/callback` redirect saved. Google OAuth client `Hiyaku Study — Supabase` was created in Google project `peppy-glyph-509413-t6` (display name `My Project 69062`). Its secret was transferred directly from the Google dialog into Supabase without adding it to source or local configuration. The Google provider is enabled, with callback `https://rmvajfhgdmzoxiuoazmd.supabase.co/auth/v1/callback` and JavaScript origin `http://localhost:3000`. The app’s Continue with Google flow successfully reaches Google’s login screen. Google audience remains in testing mode. Owner sign-in, any required test-user registration, owner bootstrap, and end-to-end persistence verification remain pending; confirm the owner’s chosen email instead of inferring it from the dashboard login. Enrollment was opened on September 22, 2026 at the owner’s request and verified by reading the saved course setting. Signed-in users can explicitly join as students; personal records remain protected by the existing policies. Reuse this project rather than creating another.
