...
Email forwarding is separate from in-app submissions. Cloudflare mail DNS replaced the five old Namecheap MX records and old SPF record on 2026-09-22. The `help@syllingo.com` rule forwards to `baconsamp@gmail.com` and is enabled, but the destination still shows Pending verification; the inbox owner must click Cloudflare’s verification email before delivery can be relied on. Do not advertise the email address as active yet. In-app submissions do not currently send email notifications.
